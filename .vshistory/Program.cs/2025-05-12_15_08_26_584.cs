    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            switch (menuChoice)
            {
                ...
                case 2:
                    GameState newGame = new GameState(game.AllPlayers);
        ...
        static void SetUpGame(GameState game, ref List<Player> playersInRound)
        {
            ...
            for(int i = 0; i < NUM_PLAYERS; i++)
            {
                ...
                //Checking if the player has been repeated (same name)
                if (player.Name == [i].name && winner.score > winnerList[i].score)
                        winnerList.RemoveAt(i); //Removing the repeated winner's old spot if the new score is higher than the old one

                else if (winner.name == winnerList[i].name && winner.score <= winnerList[i].score)
                   return; //Exiting the method if the repeated winner's new score is lower or equal than the old one
        ...
        static void GameOver(GameState game, string? winner, List<Player> playersInRound)
        {
            ...
            Player firstPlaceWinner = playersInRound[0], secondPlaceWinner = playersInRound[1];
            ...
            //Processing the total wins (Notes: Only 1st and 2nd ranked players get some points. 1st place gets 3/4 of the combined handValues of 3rd and 4th place * 2
            //while 2nd place gets 1/4. This scoring algorithm is heavily based on the UNO game app)
            int winsFirstPlace = 0 - (((handValuesList[2] + handValuesList[3]) * 2) * 3) /4;
            int winsSecondPlace = 0 - (((handValuesList[2] + handValuesList[3]) * 2) * 1) / 4;
            ...
            firstPlaceWinner.TotalWinnings += winsFirstPlace;
            secondPlaceWinner.TotalWinnings += winsSecondPlace;
