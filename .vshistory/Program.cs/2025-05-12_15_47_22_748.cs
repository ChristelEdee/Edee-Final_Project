...
namespace Edee_Final_Project
{
    ...
    internal class Program
    {
        ...
        static void AddToLeaderboard(GameState game, List<Player> playersInRound)
        {
            ...
            for (int j = 0; j < game.AllPlayers.Count; j++)
            {
                ...
                if (playersInRound[j].Name == game.AllPlayers[j].Name)
                {
                    int wins = game.AllPlayers[j].TotalWinnings;
                    playersInRound[j].TotalWinnings = wins;
                    game.AllPlayers.RemoveAt(j); //Removing temporarily (Gets added back after this loop ends)
                }

                game.AllPlayers.Add(playersInRound[j]); //Adding the player to the TOTAL player list (aka leaderboard)
            }
        }
