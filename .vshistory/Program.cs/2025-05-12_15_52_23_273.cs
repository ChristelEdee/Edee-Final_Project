...
namespace Edee_Final_Project
{
    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            List<Player> allPlayers = game.AllPlayers;
            ...
            SetUpGame(game, ref playersInRound, ref allPlayers);
            ...
            while (mainLoop)
            {
                ...
                switch (menuChoice)
                {
                    ...
                    case 2:
                        ...
                        SetUpGame(newGame, ref newPlayersList, ref allPlayers); 
        ...
        //Important methods:
        static void SetUpGame(GameState game, ref List<Player> playersInRound, ref List<Player> allPlayers)
        {
            ...
            for(int i = 0; i < NUM_PLAYERS; i++)
            {
                ...
                for(int j =0; j < allPlayers.Count; j++)
                {
                    ...
                    if(player.Name == allPlayers[j].Name)
                    {
                        player.TotalWinnings = allPlayers[i].TotalWinnings;
                        allPlayers.RemoveAt(j); //Removing temporarily (Gets added back after this loop ends)
                    }             
                }

                allPlayers.Add(player); //Adding the player to the TOTAL player list (aka leaderboard)
            }
        ...
        static void GameOver(GameState game, string? winner, List<Player> playersInRound)
        {
            ...
            int winsSecondPlace = -(((handValuesList[2] + handValuesList[3]) * 2) * 1) / 4;
