...
namespace Edee_Final_Project
{
    ...
    internal class Program
    {
        ...
        static void SetUpGame(GameState game, ref List<Player> playersInRound)
        {
            ...
            for(int i = 0; i < NUM_PLAYERS; i++)
            {
                ...
                for(int j =0; j < game.AllPlayers.Count; j++)
                {
                    ...
                    {
                        player.TotalWinnings = game.AllPlayers[i].TotalWinnings;
        ...
        static void GameOver(GameState game, string? winner, List<Player> playersInRound)
        {
            ...
            firstPlaceWinner.Name = playersInRound[0].Name;
            firstPlaceWinner.TotalWinnings += winsFirstPlace;

            secondPlaceWinner.Name = playersInRound[1].Name;
            secondPlaceWinner.TotalWinnings += winsSecondPlace;

            for(int i = 0; i < game.AllPlayers.Count; i++)
            {
                if (game.AllPlayers[i].Name == firstPlaceWinner.Name)
                    game.AllPlayers[i] = firstPlaceWinner;
                if (game.AllPlayers[i].Name == secondPlaceWinner.Name)
                    game.AllPlayers[i] = secondPlaceWinner;
            }
