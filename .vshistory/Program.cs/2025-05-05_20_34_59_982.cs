...
namespace Edee_Final_Project
{
    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            int player2Num = 2, player3Num = 3, player4Num = 4;
            ...
            Player1Turn(game);
            Display(game);
            BotTurn(game, game.PlayerHands[1], player2Num);
        ...
        static void Player1Turn(GameState game)
        {
            int count = 0;
            bool canYouPlay = CanYouPlay(game.PlayerHands[0].Cards, game.CardInMiddle, ref count);
            ...
            else
            {
                ...
                CheckValidCards(game.PlayerHands[0].Cards, game.CardInMiddle, ref validCards);
                ...
                game.PlayCard(validCards[cardNumInput - 1], game.PlayerHands[0]);
            }
        }
        static void BotTurn(GameState game, Hand botHand, int playerNum)
        {
            int count = 0;
            bool canYouPlay = CanYouPlay(botHand.Cards, game.CardInMiddle, ref count);

            Console.WriteLine($"Player {playerNum} is playing...");

            if(canYouPlay == true)
            {
                List<Card> validCards = new List<Card>();
                CheckValidCards(botHand.Cards, game.CardInMiddle, ref validCards);
            }
        }
