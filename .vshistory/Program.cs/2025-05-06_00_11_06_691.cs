        ...
        static void Reverse(GameState game, bool isClockwise, int playerNum, bool gameLoop)
        {
            if(playerNum == 1)
            {
                while(gameLoop == true || isClockwise == false)
                {
                    ...
                    PlayerTurn(game, game.PlayerHands[3], playerNum+3);

                    Display(game, playerNum+2);
                    PlayerTurn(game, game.PlayerHands[2], playerNum+2);

                    Display(game, playerNum+1);
                    PlayerTurn(game, game.PlayerHands[1], playerNum+1);

                    Display(game, playerNum);
                    PlayerTurn(game, game.PlayerHands[0], playerNum);
                }
            }
            if (playerNum == 2)
            {
                ...
                {
                    Display(game, playerNum-1);
                    PlayerTurn(game, game.PlayerHands[0], playerNum-1);

                    Display(game, playerNum + 2);
                    PlayerTurn(game, game.PlayerHands[3], playerNum + 2);

                    Display(game, playerNum + 1);
                    PlayerTurn(game, game.PlayerHands[2], playerNum + 1);

                    Display(game, playerNum);
                    PlayerTurn(game, game.PlayerHands[1], playerNum);
                }
            }
            if (playerNum == 3)
            {
                while (gameLoop == true || isClockwise == false)
                {
                    ...
                    Display(game, playerNum - 2);
                    PlayerTurn(game, game.PlayerHands[0], playerNum-2);

                    Display(game, playerNum + 1);
                    PlayerTurn(game, game.PlayerHands[3], playerNum + 1);

                    Display(game, playerNum);
                    PlayerTurn(game, game.PlayerHands[2], playerNum);
                }
            }
            if (playerNum == 4)
            {
                while (gameLoop == true || isClockwise == false)
                {
                    ...
                    Display(game, playerNum-2);
                    PlayerTurn(game, game.PlayerHands[1], playerNum-2);

                    Display(game, playerNum-3);
                    PlayerTurn(game, game.PlayerHands[0], playerNum-3);

                    Display(game, playerNum);
                    PlayerTurn(game, game.PlayerHands[3], playerNum);
                }
            }
        }
