    ...
    internal class Program
    {
        ...
        static void SetUpGame(GameState game, ref List<Player> playersInRound)
        {
            ...
            for(int i = 0; i < NUM_PLAYERS; i++)
            {
                ...
                playerNames[i] = player.Name; //Adding the player's name to the array of strings (useful for validation)

                //Making sure that the user didn't enter a player twice in the same round or doesn't enter anything
                while(playerNames.Contains(player.Name))
                {
                    if (player.Name == null)
                        Console.WriteLine("Player name cannot be blank. Enter another one: ");
                    else
                    Console.Write("That player is already in the game. Enter another one: ");
