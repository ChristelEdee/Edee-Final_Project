    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            while (gameLoop || game.DrawDeck.CardsLeft != 0)
            {
                ...
                {
                    Display(game, i+1, playersInRound);
                    PlayerTurn(game, game.PlayerHands[i], i + 1);
                }
            }
        ...
        static void PlayerTurn(GameState game, Hand playerHand, int playerNum)
        {
            ...
            int nextPlayerNum = GetNextPlayerNum(playerNum); //Useful for knowing who's the next player (in case of action cards)
