    ...
    internal class Program
    {
        ...
        static void PlayGame(GameState game, List<Player> playersInRound)
        {
            ...
            while (game.DrawDeck.CardsLeft != 0)
            {
                ...
                game.DisplayGame(currentPlayerIndex + 1, playersInRound);

                if (skipNextPlayer)
                {
                    skipNextPlayer = false; // Skip only one player
                }
                else
                {
                    PlayerTurn(game, game.PlayerHands[currentPlayerIndex], direction == 1, currentPlayerIndex + 1, out Card? lastPlayedCard);

                    // Handle Skip and Reverse
                    if (lastPlayedCard.HasValue)
                    {
                        if (lastPlayedCard.Value.Rank == Rank.Skip)
                        {
                            skipNextPlayer = true;
                        }
                        else if (lastPlayedCard.Value.Rank == Rank.Reverse)
                        {
                    ...
                    // Check for winner
                    if (turnCount >= 25 && CheckForWinner(game.PlayerHands[currentPlayerIndex]))
                    {
